...
const AudioRecorder = () => {
  ...
  const handleTranslate = async () => {
    try {
      ...
      const transURL = await tts2(translation, languageCode); // "zh-cn"
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      ...
      <PlayButton isRecording={isRecording} onToggleRecording={handleRecordingToggle} />
      {isRecording}
      <div>
        <AudioPlayer audioURL={audioURL} />
        <div>
          <div className="my-2"><h2><strong>Transcription</strong></h2></div>
          <div className="my-2"><button onClick={handleTranscribe}>Transcribe</button></div>

          {isLoading && <p>Loading...</p>}
          {error && <p>Did not transcribe file.</p>}
          {transcript && <div className="my-2"><p><strong>Transcription:</strong> {transcript}</p></div>}
          {translation && <div className="my-2"><p><strong>Translation:</strong> {translation}</p></div>}
        </div>
        {transcript !== '' && (
